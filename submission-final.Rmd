...
top_artists_summary %>%
  ...
  coord_flip() + 
  labs(x = "Artist", y = "Change in rank", title = "Changes in rank by popularity") + 
...
top_artists_summary %>%
  ...
  coord_flip() +
  labs(x = "Artist", y = "Change in rank", title = "Changes in rank by followers") +
  theme_minimal(base_size = 12, base_family = "serif") + 
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
```
...
cor_plot <- cor(top_artists_summary[, c(3:13, 23)], use = "complete.obs") %>% 
  corrplot(type = "upper", method = "number", tl.col = "black", 
           ...
           title = "Correlation matrix of the independent variables", mar=c(0,0,2,0), number.cex = 0.75) 
```
...
plot_summs(m_pop, m_fol, model.names = c("Popularity", "Followers"), 
  ...
  labs(title = "Coefficient values of linear models with confidence intervals", 
...
genre_changes %>% 
  ...
  coord_flip() + 
  labs(x = "Genre", y = "Change in rank", title = "Changes in rank by genre") + 
